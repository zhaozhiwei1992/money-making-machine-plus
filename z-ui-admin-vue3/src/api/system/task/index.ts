...
export const getTableListApi = (params: any): Promise<TableResponse<TableData>> => {
  ...
  return request.get({ url: '/system/task-params', params })
}

export const saveTableApi = (data: Partial<TableData>): Promise<IResponse> => {
  return request.post({ url: '/system/task-params', data })
}

// 获取指定数据详情
export const getTableDetApi = (id: string): Promise<IResponse<TableData>> => {
  // url?id=xx
  return request.get({ url: '/system/task-params/detail', params: { id } })
}

// 批量删除
export const delTableListApi = (ids: string[] | number[]): Promise<string> => {
  // 适配后端, 直接使用RequestBody接收数据, 不能使用{ids}, 构建为map对象, key为data
  return request.delete({ url: '/system/task-params', data: ids })
}

// 启用
export const startApi = (ids: string[] | number[]): Promise<string> => {
  return request.post({ url: '/system/task-params/start', data: ids })
}

// 停用
export const stopApi = (ids: string[] | number[]): Promise<string> => {
  return request.post({ url: '/system/task-params/stop', data: ids })
}

// 获取定时任务实现树
export const getJobSelect = async (): Promise<ComponentOptions[]> => {
  return await request.get({ url: '/system/task-params/select' })
}
