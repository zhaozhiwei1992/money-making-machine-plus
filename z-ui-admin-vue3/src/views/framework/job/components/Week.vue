  ...
  <ElForm size="small">
    ...
    <ElFormItem>
      <ElRadio v-model="radioValue" :label="3">
        ...
        <ElSelect clearable v-model="cycle01">
          <ElOption v-for="(item, index) of weekList" :key="index" :label="item.value" :value="item.key" :disabled="item.key === 1">{{
            item.value
          }}</ElOption>
        </ElSelect>
        -
    ...
    <ElFormItem>
      <ElRadio v-model="radioValue" :label="4">
        ...
        <ElSelect clearable v-model="average02">
          <ElOption v-for="(item, index) of weekList" :key="index" :label="item.value" :value="item.key">{{ item.value }}</ElOption>
        </ElSelect>
      </ElRadio>
    </ElFormItem>
    ...
    <ElFormItem>
      <ElRadio v-model="radioValue" :label="6">
        指定
        <ElSelect clearable v-model="checkboxList" placeholder="可多选" multiple style="width: 100%">
          <ElOption v-for="(item, index) of weekList" :key="index" :label="item.value" :value="String(item.key)">{{
            item.value
          }}</ElOption>
        </ElSelect>
      </ElRadio>
