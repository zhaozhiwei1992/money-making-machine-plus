...
@Component
public class MinFormulaTransformer implements CheckFormulaTransformer{
    @Override
    public String transform(String expression, Map<String, String> formatMapABC) {
        ...
        Pattern sumPattern = Pattern.compile("MIN（(.*?)）");
        ...
        while (matcher.find()) {
            ...
            matcher.appendReplacement(result, "#min（" + String.join(",", expandedCells) + "）");
        }
