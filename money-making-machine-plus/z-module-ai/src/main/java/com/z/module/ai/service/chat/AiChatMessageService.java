...
public class AiChatMessageService {
    ...
    public AiChatMessageSendRespVO sendMessage(AiChatMessageSendReqVO sendReqVO, Long userId) {
        ...
        List<AiChatMessageDO> historyMessages = chatMessageRepository.findByConversationId(conversation.getId());
        // 1.2 校验模型
    ...
    private List<AiKnowledgeSegmentSearchRespBO> recallKnowledgeSegment(String content,
        ...
        if (role == null || CollUtil.isEmpty(Collections.singleton(role.getKnowledgeIds()))) {
            return Collections.emptyList();
        }
...
//        message.setCreateTime(LocalDateTime.now());
    ...
    public void deleteChatMessage(Long id, Long userId) {
        // 1. 校验消息存在
        AiChatMessageDO message = chatMessageRepository.findById(id).orElse(null);
    ...
    public void deleteChatMessageByConversationId(Long conversationId, Long userId) {
        // 1. 校验消息存在
        List<AiChatMessageDO> messages = chatMessageRepository.findByConversationId(conversationId);
        if (CollUtil.isEmpty(messages) || ObjUtil.notEqual(messages.get(0).getUserId(), userId)) {
            throw exception(CHAT_MESSAGE_NOT_EXIST);
        }
        // 2. 执行删除
        chatMessageRepository.deleteAllById(CollectionUtils.convertList(messages, AiChatMessageDO::getId));
    }
    ...
    public Map<Long, Integer> getChatMessageCountMap(Collection<Long> conversationIds) {
        return chatMessageRepository.selectCountMapByConversationId(conversationIds);
    }

    public PageResult<AiChatMessageDO> getChatMessagePage(Pageable pageable, AiChatMessageDO query) {
        log.debug("REST request to get all AiChatMessage for page {}", pageable);
        ...
        Example<AiChatMessageDO> ex = Example.of(query, matcher);
        page = chatMessageRepository.findAll(ex, pageable);
