...
import com.z.module.ai.web.rest.admin.knowledge.vo.segment.AiKnowledgeSegmentPageReqVO;
import com.z.module.ai.web.rest.admin.knowledge.vo.segment.AiKnowledgeSegmentProcessRespVO;
import com.z.module.ai.web.rest.admin.knowledge.vo.segment.AiKnowledgeSegmentSaveReqVO;
import com.z.module.ai.web.rest.admin.knowledge.vo.segment.AiKnowledgeSegmentUpdateStatusReqVO;
...
import com.z.module.ai.repository.AiKnowledgeSegmentRepository;
...
//    @Resource
    ...
    public PageResult<AiKnowledgeSegmentDO> getKnowledgeSegmentPage(Pageable pageable, AiKnowledgeSegmentDO query) {
        ...
        Page<AiKnowledgeSegmentDO> page;
        ...
        Example<AiKnowledgeSegmentDO> ex = Example.of(query, matcher);
        page = segmentRepository.findAll(ex, pageable);
    ...
    public void createKnowledgeSegmentBySplitContent(Long documentId, String content) {
        ...
        segmentRepository.saveAll(segmentDOs);
        // 3.2 切片向量化
    ...
    public void deleteKnowledgeSegmentByDocumentId(Long documentId) {
        // 1. 查询需要删除的段落
        List<AiKnowledgeSegmentDO> segments = segmentRepository.findByDocumentId(documentId);
        if (CollUtil.isEmpty(segments)) {
            return;
        }

        // 2. 批量删除段落记录
        segmentRepository.deleteAllById(CollectionUtils.convertList(segments, AiKnowledgeSegmentDO::getId));

        // 3. 删除向量存储中的段落
        VectorStore vectorStore = getVectorStoreById(segments.get(0).getKnowledgeId());
        vectorStore.delete(CollectionUtils.convertList(segments, AiKnowledgeSegmentDO::getVectorId));
    }
    ...
    public void updateKnowledgeSegmentStatus(AiKnowledgeSegmentUpdateStatusReqVO reqVO) {
        ...
        // 3. 更新状态
        segmentRepository.save(new AiKnowledgeSegmentDO().setId(reqVO.getId()).setStatus(reqVO.getStatus()));
    ...
    public void reindexKnowledgeSegmentByKnowledgeId(Long knowledgeId) {
        ...
        List<AiKnowledgeSegmentDO> segments = segmentRepository.findByKnowledgeIdAndStatus(
    ...
    private void writeVectorStore(VectorStore vectorStore, AiKnowledgeSegmentDO segmentDO, Document segment) {
        ...
        // 2. 更新向量 ID
        segmentRepository.save(new AiKnowledgeSegmentDO().setId(segmentDO.getId()).setVectorId(segment.getId()));
    }
    ...
    private void deleteVectorStore(VectorStore vectorStore, AiKnowledgeSegmentDO segmentDO) {
        ...
        segmentRepository.save(new AiKnowledgeSegmentDO().setId(segmentDO.getId())
                .setVectorId(AiKnowledgeSegmentDO.VECTOR_ID_EMPTY));
    ...
    public List<AiKnowledgeSegmentSearchRespBO> searchKnowledgeSegment(AiKnowledgeSegmentSearchReqBO reqBO) {
        ...
        // 3.1 段落召回
        List<AiKnowledgeSegmentDO> segments = segmentRepository
                .findAllByVectorIdIn(CollectionUtils.convertList(documents, Document::getId));
        if (CollUtil.isEmpty(segments)) {
            return ListUtil.empty();
        }
        // 3.2 增加召回次数
        // TODO: Need to implement updateRetrievalCountIncrByIds in Repository
        // segmentRepository.updateRetrievalCountIncrByIds(CollectionUtils.convertList(segments, AiKnowledgeSegmentDO::getId));
    ...
    private AiKnowledgeSegmentDO validateKnowledgeSegmentExists(Long id) {
        AiKnowledgeSegmentDO knowledgeSegment = segmentRepository.findById(id).orElse(null);
    ...
    public List<AiKnowledgeSegmentProcessRespVO> getKnowledgeSegmentProcessList(List<Long> documentIds) {
        ...
        List<AiKnowledgeSegmentDO> allByDocumentIdIn = segmentRepository.findAllByDocumentIdIn(documentIds);
        return null;
    }
    ...
    public List<AiKnowledgeSegmentDO> getKnowledgeSegmentList(Collection<Long> ids) {
        ...
        return segmentRepository.findAllById(ids);
    }
